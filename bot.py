...
BUILD_VERSION = "v15-stories-patch1"
...
# =========================
# Instagram Stories viewer (browser automation)
# =========================


def _normalize_instagram_username(raw: str) -> str | None:
    value = raw.strip()
...
def _reset_stories_daily_counter(state: dict) -> bool:
    today = datetime.now(TZ).date().isoformat()
    if state.get("stories_viewed_date") != today:
        state["stories_viewed_date"] = today
        state["stories_viewed_today"] = 0
        return True
    return False


def _stories_configured() -> bool:
    return bool(IG_STORIES_LOGIN and IG_STORIES_PASSWORD)


def _stories_status_text(state: dict) -> str:
    _reset_stories_daily_counter(state)
    enabled = bool(state.get("stories_enabled"))
    usernames = list(state.get("stories_usernames", []))
    viewed = int(state.get("stories_viewed_today", 0))
    limit = int(state.get("stories_daily_limit", 30))
    return (
        "👀 АВТОПРОСМОТР STORIES\n\n"
        f"Статус: {'ВКЛЮЧЕН ✅' if enabled else 'ВЫКЛЮЧЕН ⛔️'}\n"
        f"Instagram-вход: {'НАСТРОЕН ✅' if _stories_configured() else 'НЕ НАСТРОЕН ❌'}\n"
        f"Аккаунтов в списке: {len(usernames)}\n"
        f"Сегодня просмотрено: {viewed}/{limit}\n"
        f"Интервал между аккаунтами: {IG_STORIES_INTERVAL_SECONDS} сек.\n\n"
        "Бот работает только со списком аккаунтов, который ты добавишь сам."
    )


def stories_keyboard(state: dict) -> InlineKeyboardMarkup:
    enabled = bool(state.get("stories_enabled"))
    limit = int(state.get("stories_daily_limit", 30))
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(
                ...
                callback_data="stories:toggle",
            )],
            [
                InlineKeyboardButton(text="➕ Добавить аккаунты", callback_data="stories:add"),
                InlineKeyboardButton(text="📋 Список", callback_data="stories:list"),
            ],
            [
                InlineKeyboardButton(text=f"🎯 Лимит — {limit}", callback_data="stories:limit"),
                InlineKeyboardButton(text="🧪 Просмотреть сейчас", callback_data="stories:test"),
            ],
            [InlineKeyboardButton(text="🗑 Очистить список", callback_data="stories:clear")],
        ]
    )


async def _ig_stories_login(page) -> None:
    await page.goto("https://www.instagram.com/accounts/login/", wait_until="domcontentloaded", timeout=60000)
    await page.wait_for_timeout(2500)

    # Если сессия уже авторизована, Instagram обычно уводит с /accounts/login/.
    ...
    await user.fill(IG_STORIES_LOGIN)
    await password.fill(IG_STORIES_PASSWORD)
    await password.press("Enter")
    await page.wait_for_timeout(6000)

    if "/accounts/login" in page.url:
        raise RuntimeError(
            "Instagram не принял вход. Возможно, требуется подтверждение входа/2FA. "
            "Открой Instagram вручную и проверь уведомления безопасности."
        )


async def _view_story_username(username: str) -> tuple[bool, str]:
    try:
        from playwright.async_api import async_playwright
    except ImportError as exc:
        raise RuntimeError(
            "Не установлен Playwright. Добавь пакет playwright и выполни: playwright install chromium"
        ) from exc

    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=IG_STORIES_HEADLESS)
        storage_kwargs = {}
        if IG_STORIES_STORAGE_FILE.exists():
            storage_kwargs["storage_state"] = str(IG_STORIES_STORAGE_FILE)
        context = await browser.new_context(**storage_kwargs)
        page = await context.new_page()
        try:
            await page.goto("https://www.instagram.com/", wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(2000)
            if "/accounts/login" in page.url or await page.locator('input[name="username"]').count():
                await _ig_stories_login(page)
            ...
            expected = f"/stories/{username.lower()}/"
            if expected not in current:
                return False, "Сторис сейчас не найдена или Instagram перенаправил страницу."

            # Просматриваем кадры только этого аккаунта. После последнего кадра Instagram
            # может перейти к следующему аккаунту — тогда сразу останавливаемся.
            for _ in range(12):
                await page.wait_for_timeout(3000)
                if expected not in page.url.lower():
                    break
                await page.keyboard.press("ArrowRight")

            await context.storage_state(path=str(IG_STORIES_STORAGE_FILE))
            return True, "Просмотрено"
        finally:
            await context.close()
            await browser.close()


async def view_next_story_account(bot: Bot, chat_id: int, *, notify: bool = False) -> bool:
    state = load_state()
    changed = _reset_stories_daily_counter(state)
    usernames = list(state.get("stories_usernames", []))
    limit = max(1, int(state.get("stories_daily_limit", 30)))
    viewed = int(state.get("stories_viewed_today", 0))

    if changed:
        save_state(state)
    if not usernames or viewed >= limit:
        return False
    if not _stories_configured():
        raise RuntimeError("Добавь IG_STORIES_LOGIN и IG_STORIES_PASSWORD в переменные окружения.")

    history = list(state.get("stories_history", []))
    today = datetime.now(TZ).date().isoformat()
    viewed_today_names = {
        str(x.get("username", "")).lower()
        for x in history
        if str(x.get("at", "")).startswith(today) and x.get("ok")
    }
    candidates = [u for u in usernames if u.lower() not in viewed_today_names]
    if not candidates:
        return False

    username = candidates[0]
    ok, detail = await _view_story_username(username)

    state = load_state()
    _reset_stories_daily_counter(state)
    state["stories_last_username"] = username
    state["stories_last_error"] = None if ok else detail
    if ok:
        state["stories_viewed_today"] = int(state.get("stories_viewed_today", 0)) + 1
    ...
    state["stories_history"] = history[-200:]
    save_state(state)

    if notify:
        await bot.send_message(
            chat_id,
            f"{'✅' if ok else '⚠️'} @{username}: {detail}\n"
            f"Сегодня: {state.get('stories_viewed_today', 0)}/{state.get('stories_daily_limit', 30)}"
        )
    return ok


async def instagram_stories_loop(bot: Bot) -> None:
    await asyncio.sleep(25)
    while True:
        try:
            state = load_state()
            if not state.get("stories_enabled"):
                await asyncio.sleep(30)
                continue
            owner = get_owner_id()
            if not owner:
                await asyncio.sleep(30)
                continue
            await view_next_story_account(bot, owner, notify=False)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            logging.exception("Instagram Stories loop error")
            state = load_state()
            state["stories_last_error"] = f"{type(exc).__name__}: {exc}"
            save_state(state)
        await asyncio.sleep(IG_STORIES_INTERVAL_SECONDS)
...
async def main():
    ...
    ig_stories_task = asyncio.create_task(instagram_stories_loop(bot))
    try:
        await dp.start_polling(bot)
    finally:
        task.cancel()
        ig_auto_task.cancel()
        ig_stories_task.cancel()
