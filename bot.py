...
def gradient_background(width: int, height: int) -> Image.Image:
    # Быстрый вертикальный градиент + мягкое синее свечение.
    img = Image.new("RGB", (width, height), (5, 14, 36))
    ...
    for y in range(height):
        ...
        od.line((0, y, width, y), fill=c)

    glow = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.ellipse(
        (width * 0.48, -height * 0.18, width * 1.15, height * 0.42),
        fill=(0, 125, 255, 115),
    )
...
def render_draft(
    ...
    top = 92 if story else 46
    scale = 1.15 if story else 1.0
    draw_logo(d, width, top, scale)

    subtitle = "ТОП КРАСИВЫХ НОМЕРОВ НА СЕГОДНЯ"
    sf = font(FONT_BOLD, 39 if story else 31)
    sb = d.textbbox((0, 0), subtitle, font=sf)
    d.text(
        ((width - (sb[2] - sb[0])) // 2, top + (112 if story else 90)),
        subtitle,
        font=sf,
        fill=(176, 222, 255),
    )

    panel_top = 280 if story else 190
    panel_bottom = height - (310 if story else 190)
    panel = (62, panel_top, width - 62, panel_bottom)
    draw_glow_panel(img, panel, 38)

    d = ImageDraw.Draw(img)
    line_h = (panel_bottom - panel_top - 70) // max(len(items), 1)
    number_font = font(FONT_MONO, 63 if story else 47)
    price_font = font(FONT_BOLD, 31 if story else 24)

    for i, item in enumerate(items):
        y1 = panel_top + 30 + i * line_h
        y2 = y1 + line_h - 10
        if i % 2 == 0:
            d.rounded_rectangle(
                (86, y1, width - 86, y2),
        ...
        pb = d.textbbox((0, 0), phone_text, font=number_font)
        d.text(
            (110, y1 + 16),
            phone_text,
            font=number_font,
        ...
        prb = d.textbbox((0, 0), price_text, font=price_font)
        d.text(
            (width - 110 - (prb[2] - prb[0]), y1 + (83 if story else 65)),
            price_text,
            font=price_font,
            fill=(65, 187, 255),
        )
    ...
    bf = font(FONT_BOLD, 31 if story else 23)
    bb = d.textbbox((0, 0), benefit, font=bf)
    d.text(
        ((width - (bb[2] - bb[0])) // 2, panel_bottom + 55),
        benefit,
        font=bf,
        fill=(138, 205, 255),
    )
    ...
    cf = font(FONT_BOLD, 42 if story else 30)
    cb = d.textbbox((0, 0), cta, font=cf)
    cw = cb[2] - cb[0] + 70
    ch = 92 if story else 68
    cx = (width - cw) // 2
    cy = height - (175 if story else 105)
    d.rounded_rectangle(
        (cx, cy, cx + cw, cy + ch),
        radius=ch // 2,
        fill=(18, 116, 252),
