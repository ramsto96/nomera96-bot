...
def render_reels_frame(
    ...
    img = background(width, height)
    d = ImageDraw.Draw(img)
    draw_brand(d, width, 90, True)

    hf = fit_font(d, headline.upper(), FONT_BOLD, 62, 38, width - 160)
    _center_text(d, headline.upper(), hf, width, 330, (246, 251, 255))

    box = (105, 540, width - 105, 1280)
    neon_box(img, box, 46)
    d = ImageDraw.Draw(img)

    if phone:
        lf = font(FONT_BOLD, 30)
        _center_text(d, "КРАСИВЫЙ НОМЕР", lf, width, 650, (135, 205, 255))

        phone_text = format_phone(phone)
        pf = fit_font(d, phone_text, FONT_MONO, 78, 50, width - 250)
        _center_text(d, phone_text, pf, width, 790, (255, 255, 255))

        if price is not None:
            price_text = f"{price:,}".replace(",", " ") + " ₽/мес"
            prf = font(FONT_BOLD, 44)
            pb = d.textbbox((0, 0), price_text, font=prf)
            bw = (pb[2] - pb[0]) + 80
            bh = 90
            bx = (width - bw) // 2
            by = 985
            ...
            d.text((bx + 40, by + 20), price_text, font=prf, fill=(255, 255, 255))
    else:
        big = footer or "НОМЕРА96"
        bf = fit_font(d, big, FONT_BOLD, 64, 38, width - 240)
        _center_text(d, big, bf, width, 820, (255, 255, 255))

    if footer and phone:
        ff = fit_font(d, footer, FONT_BOLD, 36, 25, width - 190)
        _center_text(d, footer, ff, width, 1420, (151, 213, 255))

    draw_cta(d, width, height, True)
    img.convert("RGB").save(target, "PNG", optimize=True)


def reels_script(items: list[NumberItem], kind: str) -> str:
