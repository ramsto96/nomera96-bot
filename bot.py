...
def render_tariff_group(
    ...
    """Карточка тарифа в стиле каруселей Номера96."""
    x1, y1, x2, y2 = box
    neon_box(img, box, 28 if not story else 34)
    d = ImageDraw.Draw(img)

    ribbon_h = 68 if not story else 84
    ribbon_w = min(int((x2 - x1) * 0.82), 430 if not story else 540)
    draw_price_ribbon(
        d,
        x1 + 14,
        y1 + 12,
    ...
    phone_top = y1 + ribbon_h + (30 if not story else 38)
    phone_bottom = y2 - 18
    usable_h = max(1, phone_bottom - phone_top)
    row_h = usable_h // max(1, len(items))

    for idx, item in enumerate(items):
        py = phone_top + idx * row_h
        if idx > 0:
            d.line(
                (x1 + 24, py - 7, x2 - 24, py - 7),
                fill=(38, 85, 140, 150),
                width=1,
            )
        ...
        pf = fit_font(
            ...
            phone,
            FONT_MONO,
            39 if not story else 49,
            23 if not story else 29,
            (x2 - x1) - 38,
        )

        first = phone[:-5]
        last = phone[-5:]
        fb = d.textbbox((0, 0), first, font=pf)
        lb = d.textbbox((0, 0), last, font=pf)
        total_w = (fb[2] - fb[0]) + (lb[2] - lb[0])
        px = (x1 + x2 - total_w) // 2

        d.text((px, py), first, font=pf, fill=(255, 255, 255))
        d.text(
            (px + (fb[2] - fb[0]), py),
            last,
            font=pf,
...
def render_card_list(
    ...
    # Лого
    draw_brand(d, width, 36 if not story else 62, story)

    # Заголовок — как на исходном референсе
    if "1000" in title:
        line1 = "КРАСИВЫЕ НОМЕРА"
        line2 = "ДО 1000 ₽"
    elif "ПРЕМИУМ" in title:
        line1 = "ПРЕМИУМ НОМЕРА"
        line2 = "В НАЛИЧИИ"
    elif "ТОП-5" in title:
        line1 = "КРАСИВЫЕ НОМЕРА"
        line2 = "В НАЛИЧИИ"
    ...
    f1 = fit_font(
        d, line1, FONT_BOLD,
        54 if not story else 72,
        30,
        width - 110,
    )
    b1 = d.textbbox((0, 0), line1, font=f1)
    y1 = 145 if not story else 205
    d.text(
    ...
    f2 = fit_font(
        d, line2, FONT_BOLD,
        70 if not story else 92,
        34,
        width - 110,
    )
    b2 = d.textbbox((0, 0), line2, font=f2)
    y2 = y1 + (62 if not story else 84)
    d.text(
    ...
    draw_subbrand(
        d,
        width,
        y2 + (88 if not story else 118),
        "ПЛАТИТЕ ТОЛЬКО ЗА ТАРИФ",
        story,
    )

    # Группируем номера по цене
    ...
    # Для TOP-5 обычно 3–4 тарифа. Сетка близка к референсу:
    # 2 колонки + широкая нижняя карточка.
    top = 360 if not story else 530
    bottom = height - (190 if not story else 300)
    left = 82
    right = width - 82
    gap = 22 if not story else 28
    ...
    if n == 1:
        ...
        mid = (left + right) // 2
        boxes = [
            (left, top, mid - gap // 2, bottom),
            (mid + gap // 2, top, right, bottom),
        ]
    elif n == 3:
        col_w = (right - left - gap) // 2
        upper_h = int((bottom - top) * 0.56)
        boxes = [
            (left, top, left + col_w, top + upper_h),
            (left + col_w + gap, top, right, top + upper_h),
            (left, top + upper_h + gap, right, bottom),
        ]
    else:
        # 4 группы: 2x2
        col_w = (right - left - gap) // 2
        row_h = (bottom - top - gap) // 2
        boxes = [
            (left, top, left + col_w, top + row_h),
            (left + col_w + gap, top, right, top + row_h),
            (left, top + row_h + gap, left + col_w, bottom),
            (left + col_w + gap, top + row_h + gap, right, bottom),
        ]

    # Если групп больше 4 — объединяем хвост в 4-ю карточку
    if len(groups) > 4:
        head = groups[:3]
        tail_items = []
        for price, its in groups[3:]:
            tail_items.extend(its)
        fake_price = groups[3][0]
        groups = head + [(fake_price, tail_items[:3])]
        boxes = boxes[:4]

    for (price, group_items), box in zip(groups, boxes):
        render_tariff_group(img, box, price, group_items, story)

    # Нижняя волна/акцент
    d = ImageDraw.Draw(img)
    wave_y = height - (205 if not story else 325)
    d.arc(
        (-150, wave_y - 90, width + 150, wave_y + 230),
        start=200,
        end=340,
        fill=(30, 117, 255),
        width=8 if not story else 11,
    )
