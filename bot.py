...
BUILD_VERSION = "v10-single-cards"
...
def render_number_day(
    ...
    draw_brand(d, width, 42 if not story else 82, story)

    title = "НОМЕР ДНЯ"
    tf = fit_font(d, title, FONT_BOLD, 92 if not story else 112, 48, width - 100)
    tb = d.textbbox((0, 0), title, font=tf)
    ty = 170 if not story else 275
    d.text(((width-(tb[2]-tb[0]))//2, ty), title, font=tf, fill=(255,255,255))

    draw_subbrand(d, width, ty + (112 if not story else 140),
                  "САМЫЙ КРАСИВЫЙ НОМЕР СЕГОДНЯ", story)

    panel = (65, 390, width-65, 790) if not story else (65, 620, width-65, 1320)
    ...
    py = panel[1] + ((panel[3]-panel[1])-(pb[3]-pb[1]))//2 - 5
    d.text(((width-(pb[2]-pb[0]))//2, py), phone, font=pf, fill=(255,255,255))

    # Тариф оставляем маленькой подписью, а не отдельным блоком.
    price = f"ТАРИФ {item.price:,} ₽/МЕС".replace(",", " ")
    sf = font(FONT_BOLD, 25 if not story else 34)
    sb = d.textbbox((0,0), price, font=sf)
    d.text(((width-(sb[2]-sb[0]))//2, panel[3]-58 if not story else panel[3]-82),
           price, font=sf, fill=(79,166,255))
...
def render_card_list(
    ...
    # Для превью режима показываем лучший номер одной чистой карточкой.
    # Остальные номера выдаются отдельными карточками после одобрения.
    render_single_number(items[0], target, title, story)
