...
STORY_FIVE_TEMPLATE_PATH = Path(__file__).resolve().parent / "nomera96_story_5_template.png"
...
BUILD_VERSION = "v19.4-story-template-path-fixed"
...
async def stories_menu(message: Message, bot: Bot):
    if await deny(message):
        return

    catalog = await get_live_catalog(bot, message.chat.id, force=False)
    if not catalog:
        return

    state = load_state()
    used = set(state.get("used", []))
    items = select_story_five(catalog, used)

    if len(items) < 5:
        await message.answer("⚠️ Не удалось подобрать 5 номеров для Stories.")
        return

    folder = output_folder("story_manual")
    story_path = folder / "story.png"

    try:
        await asyncio.to_thread(
            render_five_number_story,
            items,
            story_path,
        )
    except Exception as exc:
        logging.exception("Ошибка генерации Stories")
        await message.answer(
            f"⚠️ Не удалось создать Stories. Ошибка: {type(exc).__name__}: {exc}"
        )
        return
    ...
    await bot.send_photo(
        message.chat.id,
        FSInputFile(story_path),
        caption=(
            "👀 Stories: 5 красивых номеров\n\n"
            "📱 Опубликовать Stories — отправить в Instagram\n"
            "🔄 Другие номера — собрать новую подборку"
        ),
        reply_markup=kb,
    )


@router.callback_query(F.data == "storyfive:other")
async def story_five_other(callback: CallbackQuery, bot: Bot):
    if not callback.from_user or not claim_or_check_owner(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return

    await callback.answer("Подбираю другие номера…")
    chat_id = callback.message.chat.id if callback.message else callback.from_user.id
    ...
    state = load_state()
    used = set(state.get("used", []))
    previous = set((state.get("last_approved") or {}).get("phones", []))
    used.update(previous)

    items = select_story_five(catalog, used)
    if len(items) < 5:
        await bot.send_message(chat_id, "⚠️ Не удалось подобрать другую пятёрку.")
        return
    ...
    await bot.send_photo(
        chat_id,
        FSInputFile(story_path),
        caption="👀 Новая подборка для Stories",
        reply_markup=kb,
    )
